use super::{Capabilities, CommentView, FeedQuery, LemmyApi, LoginRequest, MutationResult, Page, PostDetail, PostView, SiteInfo};
...
impl HttpLemmyApi {
    ...
    pub fn with_timeout(timeout: Duration) -> Result<Self> {
        ...
        Ok(Self { client, timeout, base_url: None, fixture_server: None })
    }
    ...
    async fn send_json(&self, request: reqwest::RequestBuilder, operation: &str, retry_read: bool) -> Result<Value> {
        for attempt in 0..MAX_READ_ATTEMPTS {
            let request = request.try_clone().ok_or_else(|| AppError::Network(format!("{operation}: request could not be cloned")))?;
        ...
        Err(AppError::Network(format!("{operation}: request attempts exhausted")))
    }

    async fn mutation_request(&self, request: reqwest::RequestBuilder, operation: &str) -> Result<Value> {
        self.send_json(request, operation, false).await
    }

    pub fn request_timeout(&self) -> Duration { self.timeout }
}
...
async fn parse_response(response: Response, operation: &str, mutation: bool) -> Result<Value> {
    ...
    if !status.is_success() {
        ...
        return Err(match status {
            ...
            StatusCode::NOT_FOUND => AppError::Authorization(format!("{message}; this Lemmy capability may be unsupported")),
...
fn server_detail(body: &str) -> String {
    serde_json::from_str::<Value>(body)
        .ok()
        .and_then(|value| value.get("error").or_else(|| value.get("message")).and_then(Value::as_str).map(ToOwned::to_owned))
        .unwrap_or_else(|| if body.trim().is_empty() { "empty server response".into() } else { body.trim().into() })
}
...
fn normalize_post(value: &Value) -> Result<PostView> {
    ...
    Ok(PostView {
        ...
        title: string(post, "name").or_else(|| string(post, "title")).unwrap_or_default(),
...
impl LemmyApi for HttpLemmyApi {
    ...
    async fn feed(&self, ctx: &ProfileContext, query: FeedQuery) -> Result<Page<PostView>> {
        let mut request = self.client.get(self.endpoint(ctx, "post/list")?).query(&[("sort", query.sort), ("type_", "All".into())]);
        if let Some(page) = query.page { request = request.query(&[("page", page)]); }
        if let Some(limit) = query.limit { request = request.query(&[("limit", limit)]); }
        if let Some(community) = query.community { request = request.query(&[("community_id", community.0)]); }
        if let Some(search) = query.search { request = request.query(&[("q", search)]); }
        let response = self.read_json(self.auth_request(request, ctx), "feed").await?;
        let items = response.get("posts").and_then(Value::as_array).ok_or_else(|| AppError::Network("feed: response did not contain posts".into()))?.iter().map(normalize_post).collect::<Result<Vec<_>>>()?;
        let next_page = response.get("next_page").and_then(Value::as_u64).map(|value| value as u32);
        Ok(Page { items, next_page })
    }

    async fn post(&self, ctx: &ProfileContext, id: PostId) -> Result<PostDetail> {
        let request = self.auth_request(self.client.get(self.endpoint(ctx, "post")?).query(&[("id", id.0)]), ctx);
        let response = self.read_json(request, "post").await?;
        let post = normalize_post(response.get("post_view").unwrap_or(&response))?;
        let comments = response.get("comments").and_then(Value::as_array).map(|comments| comments.iter().map(|comment| normalize_comment(comment, post.id)).collect()).unwrap_or_default();
    ...
    async fn login(&self, request: LoginRequest) -> Result<Session> {
        ...
        let user_id = response.get("registration_application" ).and_then(|_| None).unwrap_or_else(|| response.get("person_id").and_then(Value::as_i64)).unwrap_or_default();
        Ok(Session { token: crate::domain::SecretString::from(token), user_id: UserId(user_id) })
    }
...
fn mutation_request_parts(mutation: &Mutation) -> Result<(&'static str, Value)> {
    let result = match mutation {
        Mutation::VotePost { id, score } => ("post/like", json!({ "post_id": id.0, "score": score })),
        Mutation::VoteComment { id, score } => ("comment/like", json!({ "comment_id": id.0, "score": score })),
        Mutation::SavePost { id, saved } => ("post/save", json!({ "post_id": id.0, "save": saved })),
        Mutation::Subscribe { community, subscribed } => ("community/follow", json!({ "community_id": community.0, "follow": subscribed })),
        Mutation::CreatePost(request) => ("post", json!({ "community_id": request.community.0, "name": request.name, "body": request.body, "url": request.url.as_ref().map(Url::as_str) })),
        Mutation::EditPost(request) => ("post", json!({ "post_id": request.id.0, "name": request.name, "body": request.body, "url": request.url.as_ref().map(Url::as_str) })),
        Mutation::DeletePost(id) => ("post/delete", json!({ "post_id": id.0, "deleted": true })),
        Mutation::CreateComment(request) => ("comment", json!({ "post_id": request.post.0, "content": request.content })),
        Mutation::EditComment(request) => ("comment", json!({ "comment_id": request.id.0, "content": request.content })),
        Mutation::DeleteComment(id) => ("comment/delete", json!({ "comment_id": id.0, "deleted": true })),
    };
    Ok(result)
}
